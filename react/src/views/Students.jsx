...
export default function Students() {
    ...
    const onDeleteClick = (id) => {
        if (window.confirm('Estas seguro de que quieres eliminar a este alumno?')) {
            axiosClient.delete(`/user/${id}`)
                .then(() => {
                    getUsers()
                    showToast('El alumno fue exitosamente eliminado')
                })
        }
    }
        ...
        >
            ...
            {!loading && <div className="overflow-hidden rounded-md px-2 ">
                <div className="space-y-6 rounded-md bg-white px-5 py-2">
                    {users.length ? (<ul role="list" className="divide-y divide-gray-100">
                        {users.map(user => (
                            <li key={user.email} className="flex justify-between gap-x-6 py-5">
                                ...
                                <div className="flex flex-col items-end">
                                    ...
                                    <div className='flex'>
                                        <TButton to={`/student/${user.id}`} circle link color='indigo'>
                        ...
                        <div className="text-center text-lg">Actualmente no tienes ningun alumno.</div>
                    )}
                </div>
            </div>}
        </PageComponent>
